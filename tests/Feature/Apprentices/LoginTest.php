...
test('apprentice can login if approved and redirects to /', function () {
...
test('apprentice cannot login if status is pending', function () {
    Role::firstOrCreate(['name' => 'Aprendiz']);
    $user = User::factory()->create([
        'status' => 'pending',
        'type_document' => 'CC',
    ...
    $response->assertSessionHas('status', 'Espera a que el instructor de la ficha que selecionaste te de aceptacion para ingresar al sistema');
});
